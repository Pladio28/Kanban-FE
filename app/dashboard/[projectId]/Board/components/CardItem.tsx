...
import { CardType } from "../hooks/useLocalKanban";
...
export default function CardItem({ id, card, onOpen, onDelete, isOverlay }: Props) {
  ...
  const style: React.CSSProperties = {
    ...
    transition,
    zIndex: isDragging || isOverlay ? 9999 : undefined,
    boxShadow: isDragging || isOverlay ? "0 12px 30px rgba(2,6,23,0.12)" : undefined,
    cursor: "grab",
  };
  ...
  return (
    ...
    >
      ...
      <button onClick={() => onDelete(card.id)} className="text-red-500 p-1 hover:bg-red-50 rounded" aria-label={`Hapus ${card.title}`}>
